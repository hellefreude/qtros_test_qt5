...
void MainWindow::showButtonTestMessage() {
    QMessageBox msgBox;
    msgBox.setText("Start Publisher ...");
    msgBox.exec();
}
...
void MainWindow::on_button_test_clicked(bool check) {
    qnode.test_msg.data = "s";
    qnode.test_pub.publish(qnode.test_msg);
    ROS_INFO_STREAM("test_pub published : "<< qnode.test_msg.data);
    qnode.log(QNode::Info, "test_pub published : "+qnode.test_msg.data);
}
